...
def learnPredictor(trainExamples: List[Tuple[Any, int]], validationExamples: List[Tuple[Any, int]],
    ...
    for epoch in range(numEpochs):
        for x, y in trainExamples:
            ...
            h = sigmoid(dotProduct(weights, d))
            increment(weights, (-1)*alpha*(h-y), d)

        def predictor(text):
            return 1 if dotProduct(weights, featureExtractor(text)) >= 0 else -1

        error_trn = evaluatePredictor(trainExamples, predictor)
        print('Training Error: ('+str(epoch)+' epoch): ', error_trn)
        error_val = evaluatePredictor(validationExamples, predictor)
        print('Validation Error: (' + str(epoch) + ' epoch): ', error_val)
    # END_YOUR_CODE
    return weights
...
def generateDataset(numExamples: int, weights: WeightVector) -> List[Example]:
    ...
    def generateExample() -> Tuple[Dict[str, int], int]:
        ...
        """
        # BEGIN_YOUR_CODE (our solution is 4 lines of code, but don't worry if you deviate from this)
        r_list = random.choices(list(weights.items()), k=random.randint(1, len(weights)))
        phi = {}
        for ele in r_list:
            phi[ele[0]] = int(ele[1])
        number = dotProduct(weights, phi)
        y = 1 if number >= 0 else -1
        # END_YOUR_CODE
        return phi, y
