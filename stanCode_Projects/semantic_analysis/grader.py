...
File: grader.py
-----------------------------
Milestone 4 answer: Train error continuously decreases as the number of epoch increases.
Validation error decreases till the number of epoch reaches to 70 but it start increasing
if the number of epoch keeps going to 80, 90, and above.
...
# TODO: With the same number of epoch, the training error is lower in extractCharacterFeatures but validation error
#  is higher. And validation error in extractWordFeatures steadily decreases but it goes back and forth in
#  extractCharacterFeatures. Probably the alpha value should be lower for extractCharacterFeatures cause the update each
#  time is too big.
############################################################
